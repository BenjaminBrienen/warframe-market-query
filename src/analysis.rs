...
pub fn build_relic_values(
    ...
    for part in prime_parts {
        ...
        let Some(sources) = dropsources_by_slug.get(&part.slug) else { continue };

        for src in sources {
            if src.source_type != "relic" { continue; }
            let Some(ref relic_id) = src.relic  else { continue };
            let Some(ref rates)    = src.rates   else { continue };

            contents
                .entry(relic_id.clone())
                .or_default()
                .push((ducats, rates.intact, rates.exceptional, rates.flawless, rates.radiant));
        }
    }
    ...
    contents
        ...
        .map(|(relic_id, parts)| {
            let value = RelicValue {
                intact:      parts.iter().map(|&(d, r, _, _, _)| d as f32 * r / 100.0).sum(),
                exceptional: parts.iter().map(|&(d, _, r, _, _)| d as f32 * r / 100.0).sum(),
                flawless:    parts.iter().map(|&(d, _, _, r, _)| d as f32 * r / 100.0).sum(),
                radiant:     parts.iter().map(|&(d, _, _, _, r)| d as f32 * r / 100.0).sum(),
            };
            (relic_id, value)
        })
        .collect()
...
pub fn build_mission_scores(
    ...
    for relic in relics {
        let Some(value) = relic_values.get(&relic.id) else { continue };
        let Some(sources) = dropsources_by_slug.get(&relic.slug) else { continue };

        for src in sources {
            if src.source_type != "mission" { continue; }
            let Some(ref loc_id) = src.location else { continue };
            let Some(ref mis_id) = src.mission  else { continue };
    ...
    let mut result: Vec<MissionScore> = scores
        .into_iter()
        .map(|((location_id, mission_id, rotation), expected_ducats)| MissionScore {
            location_id,
            mission_id,
            rotation,
            expected_ducats,
